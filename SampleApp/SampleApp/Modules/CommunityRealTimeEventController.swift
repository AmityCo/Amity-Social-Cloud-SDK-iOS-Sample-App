//
//  CommunityRealTimeEventController.swift
//  SampleApp
//
...
class CommunityRealTimeEventController: UIViewController {
    
    let tableView = UITableView()
    
    var manager: CommunityRealTimeEventManager!
    ...
    override func viewDidLoad() {
        ...
        manager.fetchAllPostsForCommunity { [weak self] in
            self?.tableView.reloadData()
        }
        
        // Observe community changes too
        manager.observeCommunityChanges = { [weak self] in
            self?.tableView.reloadSections([0], with: .automatic)
        }
    }
    
    func showAllCommunityEvents(isSubscribeAction: Bool) {
        var actions = [UIAlertAction]()
        
        let events: [AmityCommunityEvent] = [.community, .posts , .postsAndComments, .comments]
        for event in events {
    ...
    func getEventDescription(event: AmityCommunityEvent) -> String {
        switch event {
        case .community:
            return "Community"
        case .posts:
...
extension CommunityRealTimeEventController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .header:
            ...
            cell.modelDetailLabel.text = manager.community.modelDescription
            
            cell.subscribeAction = { [weak self] in
                self?.showAllCommunityEvents(isSubscribeAction: true)
            }
            cell.unsubscribeAction = { [weak self] in
                self?.showAllCommunityEvents(isSubscribeAction: false)
            }
