...
struct AmityMentionUserModel {
    ...
    init(userId: String, displayName: String, avatarURL: String, isGlobalBan: Bool, isChannel: Bool) {
        self.userId = userId
...
final internal class AmityMentionManager {
    ...
    private var users: [AmityMentionUserModel] = []
    private lazy var mentionees: AmityMentioneesBuilder = AmityMentioneesBuilder()
    
    // Private community
    private lazy var privateCommunityRepository: AmityCommunityRepository = AmityCommunityRepository(client: AmityManager.shared.client!)
    private var privateCommunityMembersCollection: AmityCollection<AmityCommunityMember>?
    
    // User repository
    private lazy var userRepository: AmityUserRepository = AmityUserRepository(client: AmityManager.shared.client!)
    ...
    private var mentionConfiguration: AmityMentionConfigurations? = AmityManager.shared.client!.mentionConfigurations
    
    // Channel user
    private var membersCollection: AmityCollection<AmityChannelMember>?
    private var membershipsToken: AmityNotificationToken?
    private let channelPartecipation: AmityChannelParticipation?
    ...
    init(withType type: AmityMentionManagerType) {
        ...
        channelPartecipation = AmityChannelParticipation(client: AmityManager.shared.client!, andChannel: channelId)
        default:
            channelPartecipation = nil
        }
    }
}
...
extension AmityMentionManager {
    ...
    func addMention(from textInput: UITextInput, in text: String, at indexPath: IndexPath) {
        ...
        let type: AmityMessageMentionType = member.isChannel ? .channel : .user
    ...
    func loadMore() {
        switch type {
        case .message:
            membersCollection?.nextPage()
        case .comment(let communityId), .post(let communityId):
...
private extension AmityMentionManager {
    func search(withText text: String) {
        ...
        case .comment(let communityId), .post(let communityId):
            ...
            searchForPrivateCommunity(communityId: communityId, displayName: text)
        }
    }
    
    private func searchForChannelMembers(_ displayName: String) {
        let builder = AmityChannelMembershipFilterBuilder()
        builder.add(filter: .member)
        builder.add(filter: .mute)
        
        membersCollection = channelPartecipation?.searchMembers(displayName: displayName, filterBuilder: builder, roles: [])
        
        membershipsToken = membersCollection?.observe { [weak self] collection, _, _ in
            self?.handleSearchResponse(with: collection)
        }
    }
    
    private func searchForUsers(_ displayName: String) {
        usersCollection = userRepository.searchUser(displayName, sortBy: .displayName)
        collectionToken = usersCollection?.observe { [weak self] (collection, _, error) in
            self?.handleSearchResponse(with: collection)
        }
    }
    
    private func searchForPrivateCommunity(communityId: String, displayName: String) {
        privateCommunityMembersCollection = privateCommunityRepository.searchMembers(communityId: communityId, displayName: displayName, membership: [.member], roles: [], sortBy: .lastCreated)
    ...
    func handleSearchResponse<T>(with collection: AmityCollection<T>) {
        ...
        case .fresh:
            ...
            switch type {
            case .message:
                if let mentionConfiguration = mentionConfiguration, mentionConfiguration.isMentionAllEnabled {
                    users.insert(AmityMentionUserModel(userId: "", displayName: "all", avatarURL: "", isGlobalBan: false, isChannel: true), at: 0)
                }
                break
            default: break
            }
...
extension AmityMentionManager {
    static func getAttributedString(text: String,withMetadata metadata: [String: Any], mentionees: [AmityMentionees]) -> NSAttributedString {
        let attributedString = NSMutableAttributedString(string: text)
        let mentions = AmityMentionMapper.mentions(fromMetadata: metadata)
        if mentions.isEmpty || mentionees.isEmpty { return attributedString }
